        ...
        <div className="relative">
          ...
          <div className="space-y-3 md:space-y-3">
            {experiences.map((exp, index) => (
              <div key={index} className="relative flex gap-2 md:gap-3">
                {/* Timeline dot */}
                ...
                <div className="flex-1 min-w-0 pb-1">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-0.5 sm:gap-2">
                    <div className="min-w-0">
                      <p className="text-xs md:text-sm font-semibold text-black/85 leading-tight">{exp.title}</p>
                      <div className="flex items-center gap-1 mt-0.5">
                        <MapPin className="h-2.5 w-2.5 md:h-3 md:w-3 text-black/30 shrink-0" />
                        <p className="text-[10px] md:text-xs text-black/50 truncate">{exp.location}</p>
                      </div>
                    </div>
                    <span className={`text-[9px] md:text-[10px] font-medium px-1.5 md:px-2 py-0.5 rounded-full shrink-0 w-fit ${
                      exp.current
                  ...
                  <p className="text-[10px] md:text-[11px] text-black/60 mt-1 leading-relaxed line-clamp-2 md:line-clamp-none">{exp.description}</p>

                  {exp.tags && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {exp.tags.map((tag) => (
                        <span key={tag} className="px-1.5 py-0.5 bg-black/5 text-black/50 text-[8px] md:text-[9px] font-medium rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
